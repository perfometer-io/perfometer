...
data class SummaryData(val name: String,
...
class ScenarioSummary(
    ...
    private fun generateSummary(name: String, statistics: Collection<RequestStatistics>): SummaryData {
        val sortedTimes = statistics.map { it.timeTaken }.sorted()
        return SummaryData(name,
                           statistics.count(),
                           statistics.filter { !it.httpStatus.isSuccess }.count(),
                           sortedTimes.first(),
                           statistics.map { it.timeTaken.toMillis() }.average().let { Duration.ofMillis(it.toLong()) },
